	...
	TRY
		...
		IF internet() THEN
			...
			* Instalacion de componentes
			...
			&& Verifico y acomodo datos que pueden llegar a faltar.
			verificar_impImInMin_en_articulos()
			
			&& Agrego menu
			*WAIT WINDOW "Chequeando menues..." NOWAIT
			*agregarMenu("4.19", "Promociones", "DO FORM frm_abm_promociones", .T., 4, "", 4019)
			
			* Agrego el menú para testar envíos de mails
			*agregarMenu('11.13', 'Testear Mail', 'DO FORM frm_test_mail', .T., 11, "", 11013)
...
PROCEDURE actualizar_bd
	...
	**
	...
	upd_crear_tablas_ivacompras()
	upd_cargar_citi_alicuotas()
	upd_cambiar_tabla_citi_alic()
	...
	**
	...
	IF getGlobalCFG("MODODESA") THEN
		...
		** Agrego parámetros para sincronización de tickets en forma local o remota
		...
		addOrUpdateGlobalCFG("FETKDIRFTP", "C", "", .T.)
	ELSE
		addOrUpdateGlobalCFG("UPDSQLLOC", "L", "false", .T.)
		addOrUpdateGlobalCFG("UPD_URL", "C", "http://www.linfow.com.ar/linfow_gestion_upgrades/tito/", .T.)
		
		** Agrego parámetros para sincronización de tickets en forma local o remota
		addOrUpdateGlobalCFG("FELOCAL", "L", "true", .T.)
		addOrUpdateGlobalCFG("FETKDIRFTP", "C", "", .T.)
		addOrUpdateGlobalCFG("PTOINCIVA", "L", "true", .F.)
	ENDIF
	
	addOrUpdateGlobalCFG("UPD_EXEC1", "C", "lfwgest.exe", .F.)
	addOrUpdateGlobalCFG("UPD_EXEC2", "C", "lfwgest_dmo.exe", .F.)
	
	* Agrego parámetro para directorio temporal
	addOrUpdateGlobalCFG("TEMPDIR", "C", "C:\siscom1.2\linfow_gestion_c1\intercambio\", .F.)
	
	* Agrego parámetros para FTP
	addOrUpdateGlobalCFG("FTPHOST", "C", "", .F.)
	addOrUpdateGlobalCFG("FTPUSER", "C", "", .F.)
	addOrUpdateGlobalCFG("FTPPASS", "C", "", .F.)
	addOrUpdateGlobalCFG("FTPPORT", "C", "", .F.)
	addOrUpdateGlobalCFG("FTPINIDIR", "C", "", .F.)
	addOrUpdateGlobalCFG("USA_SFTP", "L", "", .F.)
	
	* URL de actualizador
	addOrUpdateGlobalCFG("URLUPDATER", "C", "", .F.)
	addOrUpdateGlobalCFG("FCIMPDNI", "C", "92720", .T.)
	addOrUpdateGlobalCFG("TKT_CORTE", "L", "true", .F.)
ENDPROC
...
PROCEDURE upd_crear_tablas_ivacompras
	***************************************************************
	* Permite instalar las estructuras que se requieren para 
	* generar los archivos de compras.
	***************************************************************
	LOCAL lcSql
	LOCAL loRes
	
	TEXT TO lcSql NOSHOW
		CREATE TABLE IF NOT EXISTS `citicpas_cbtes` (  
	...
	lcSql = "SELECT COUNT(*) AS cantidad FROM citicpas_cbtes"
	loRes.ActiveConnection = goConn.ActiveConnection
	loRes.Cursor_Name = "cur_x"
	loRes.OpenQuery(lcSql)
	

	IF loRes.convert_count_to_numeric(cur_x.cantidad) = 0 THEN	
		lcSql = "LOCK TABLES `citicpas_cbtes` WRITE;"
		ejecutar_comando(lcSql)
	...
	TEXT TO lcSql NOSHOW
		CREATE TABLE IF NOT EXISTS `citicpa_cab` (  `idCitiCpa` int(11) NOT NULL AUTO_INCREMENT, `idCompraC` int(11) NOT NULL, 
			`fecEmis` varchar(8) NOT NULL,  `tipoCbte` varchar(3) NOT NULL,  `ptoVta` varchar(5) NOT NULL,  
			`nroCbte` varchar(20) NOT NULL,  `nroDespImp` varchar(16) NOT NULL,  `codDoc` varchar(2) NOT NULL,  
			`cuitPV` varchar(20) NOT NULL,  `razSocPV` varchar(30) NOT NULL,  `impTotOper` varchar(15) NOT NULL,  
			`ImpConceptos` varchar(15) NOT NULL,  `ioe` varchar(15) NOT NULL,  `impPercValAg` varchar(15) NOT NULL,  
			`impPercOIN` varchar(15) NOT NULL,  `impPerIIBB` varchar(15) NOT NULL,  `impPerImpMun` varchar(15) NOT NULL,  
			`impImpInt` varchar(15) NOT NULL,  `codMon` varchar(3) NOT NULL,  `tipoCbio` varchar(10) NOT NULL,  
			`cantAlicIVA` varchar(1) NOT NULL,  `codOper` varchar(1) NOT NULL,  `credFisComp` varchar(15) NOT NULL,  
			`otrosTrib` varchar(15) NOT NULL,  `cuitEmisCorr` varchar(11) NOT NULL,  `denomEmisCorr` varchar(30) NOT NULL,  
			`ivaCom` varchar(15) NOT NULL,  `procesado` bit(1) NOT NULL DEFAULT b'0',  
			`fecProceso` datetime NOT NULL DEFAULT current_timestamp(),  PRIMARY KEY (`idCitiCpa`),  
			UNIQUE KEY `idCitiCpa` (`idCitiCpa`),  KEY `citicpa_idCompraC` (`idCompraC`)) 
			ENGINE=InnoDB AUTO_INCREMENT=0 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci
	ENDTEXT
	ejecutar_comando(lcSql)
	
	TEXT TO lcSql NOSHOW
		CREATE TABLE IF NOT EXISTS `citicpa_alic` (  
			`idCitiCpaAlic` int(11) NOT NULL AUTO_INCREMENT,  `idCompraC` int(11) NOT NULL,  
			`tipoCbte` varchar(3) NOT NULL,  `ptoVta` varchar(5) NOT NULL,  `nroCbte` varchar(20) NOT NULL,  
			`codDocVdor` varchar(2) NOT NULL,  `cuitPV` varchar(20) NOT NULL,  `impNetoGrav` double NOT NULL,  
			`alicIVA` varchar(4) NOT NULL,  `impLiq` double NOT NULL,  PRIMARY KEY (`idCitiCpaAlic`),  
...
PROCEDURE upd_cargar_citi_alicuotas
	*****************************************************
	* Carga las alícuotas según la tabla de AFIP.
	*****************************************************
	LOCAL lcSql
	LOCAL loRes
	
	loRes = CREATEOBJECT("odbc_result")
	lcSql = "SELECT COUNT(*) AS cantidad from citi_alicuotas"
	loRes.ActiveConnection = goConn.ActiveConnection
	loRes.Cursor_Name = "cur_x"
	loRes.OpenQuery(lcSql)
	
	IF loRes.convert_count_to_numeric(cur_x.cantidad) = 0 THEN
		lcSql = "LOCK TABLES `citi_alicuotas` WRITE;"
		ejecutar_comando(lcSql)
		TEXT TO lcSql NOSHOW
...
PROCEDURE upd_cambiar_tabla_citi_alic
	LOCAL lcSql
	
	lcSql = "ALTER TABLE citicpa_alic CHANGE COLUMN impNetoGrav impNetoGrav VARCHAR(15)"
	ejecutar_comando(lcSql)
	lcSql = "ALTER TABLE citicpa_alic CHANGE COLUMN impLiq impLiq VARCHAR(15)"
	ejecutar_comando(lcSql)
ENDPROC
