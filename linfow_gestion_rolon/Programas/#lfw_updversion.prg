	...
	**
	...
	TRY
		...
		IF internet() THEN
			...
			* Habilitar cuando necesite el módulo de promociones
			* WAIT WINDOW "Chequeando menues..." NOWAIT
			* agregarMenu("4.19", "Promociones", "DO FORM frm_abm_promociones", .T., 4, "", 4019)
			
			agregarMenu("4.20", "Reimprimir remitos", "DO FORM frm_reimprimir_remitos", .T., 4, "", 4020)
...
PROCEDURE agregarOActualizarParametros
	...
	**
	...
	IF getGlobalCFG("MODODESA") THEN
		...
		addOrUpdateGlobalCFG("UPD_URL", "C", "http://localhost/upgrades/rolon/", .T.)
	...
	ELSE
		...
		addOrUpdateGlobalCFG("UPD_URL", "C", "http://www.linfow.com.ar/linfow_gestion_upgrades/rolon/", .T.)
		** Agrego parámetros para sincronización de tickets en forma local o remota
		addOrUpdateGlobalCFG("FELOCAL", "L", "true", .T.)
		addOrUpdateGlobalCFG("FETKDIRFTP", "C", "", .T.)
	ENDIF
	
	addOrUpdateGlobalCFG("UPD_EXEC1", "C", "linfow_gestionc1.exe", .F.)
	addOrUpdateGlobalCFG("UPD_EXEC2", "C", "linfow_gestionc2.exe", .F.)
	
	* Parametros de esta versión
	agregarParametrosEmpresa()
	
	* Agrego parámetro para directorio temporal
	addOrUpdateGlobalCFG("TEMPDIR", "C", "C:\siscom1.2\SISCOM_MGC1\intercambio\", .F.)
...
**********************************************************************
* Agrega campos a la tabla cheques
**********************************************************************
...
***********************************************************************
* Agrega campos a la tabla rccob_d
***********************************************************************
PROCEDURE agregarCamposTablarccob_d
	LOCAL lcTabla
	LOCAL lcCampo
	...
	lcCampo = "nroTransf"
	IF !existe_campo(lcTabla, lcCampo) THEN
		lcSql = "ALTER TABLE " + lcTabla + " ADD " + lcCampo + " varchar(20) NULL"
		ejecutar_comando(lcSql)
	ENDIF
ENDPROC
