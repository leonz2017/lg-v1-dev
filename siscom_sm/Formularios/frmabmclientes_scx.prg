...
idcliente = 0
cnombretabla = clientes
listctrlsource = IdCliente,RazSoc,NomFant,telefono,nroCUIT
listwidthcols = 70,200,200,150,100
obj_dataservicename = datObject
pk_fieldname = idCliente
titulos_cabeceras = Código,Razón Social, Nombre Fantasía, Telefono,C.U.I.T.
Name = "FRMABMCLIENTES"
...
Pgf.Page1.grdDatos.Height = 523
Pgf.Page1.grdDatos.Left = 2
Pgf.Page1.grdDatos.Top = 3
Pgf.Page1.grdDatos.Width = 806
...
Pgf.Page2.cnt_fields.Height = 513
Pgf.Page2.cnt_fields.TabIndex = 16
...
lcSql = "select idcliente from ventascab where idcliente = " + ALLTRIM(STR(cur_tempo.idcliente))
loData = CREATEOBJECT("odbc_result")
loData.ActiveConnection = goConn.ActiveConnection
loData.cursor_name = "vcab"
loData.OpenQuery(lcSql)

SELECT vcab
IF RECCOUNT() > 0
	MESSAGEBOX("No se puede eliminar el cliente debido a que tiene comprobantes vinculados", 0+64, thisform.Caption)
	RETURN .F.
ENDIF
...
LOCAL loLoc, loPcia, loPais, lcSql
...
SELECT cur_Localidad
lcSql = "select * from paises where idpais = " + ALLTRIM(STR(cur_Localidad.idpais))
...
SELECT cur_pais
thisform.pgf.page2.cnt_fields.txtpais.Value = cur_pais.descripcio

loPais.close_query()
loPcia.close_query()
loLoc.close_query()

&& Agrego este ajuste porque cuando se carga un cliente
&& desde el alta rápida el nombre de fantasía se está
&& cargando como NULL.
thisform.pgf.page2.cnt_fields.txtNomFant.Value = IIF(ISNULL(cur_Tempo.nomFant), "", cur_Tempo.nomFant)

&& Foco inicial al editar
thisform.pgf.page2.cnt_fields.sel_tipoDoc.txtCodigo.SetFocus()

&& Manejo de la máscara de entrada del número de CUIT / DNI
IF ALLTRIM(thisform.pgf.page2.cnt_fields.sel_tipoDoc.txtCodigo.Value) == "CUIT" ; 
		.OR. ALLTRIM(thisform.pgf.page2.cnt_fields.sel_tipoDoc.txtCodigo.Value) == "CUIL" THEN
	thisform.pgf.page2.cnt_fields.txtNroCuit.InputMask = "XX-XXXXXXXX-X"
ELSE
	thisform.pgf.page2.cnt_fields.txtNroCuit.InputMask = ""
ENDIF
ENDPROC
...
lcSql = "SELECT nroCuit FROM clientes WHERE nrocuit = '" + ALLTRIM(thisform.pgf.page2.cnt_fields.txtNroCuit.Value) + "' AND idcliente <> " + ALLTRIM(STR(thisform.idcliente))
loResult.ActiveConnection = goConn.ActiveConnection
loResult.cursor_name = "cur_cliente"
loResult.OpenQuery(lcSql)

SELECT cur_cliente
IF RECCOUNT() > 0
	IF MESSAGEBOX("El número de Cuit ingresado ya se encuentra cargado a otro cliente, desea grabarlo igualmente?",4+32,thisform.Caption) != 6
		loResult.close_query()
		RETURN .F.
	ENDIF 
ENDIF 

*IF (Thisform.pgf.page2.cnt_fields.chkCtrlCM.Value = .T.) THEN
*	IF Thisform.pgf.page2.cnt_fields.txtCredMaximo.Value = 0 THEN
*		thisform.msgerror = "Atención, ha indicado de que desea controlar el crédito máximo, " + ;
*			"debe ingresar el importe de crédito máximo que se le otorga"
*		Thisform.pgf.page2.cnt_fields.txtCredMaximo.SetFocus()
*		RETURN .F.
*	ENDIF
*ENDIF
...
RETURN .T.
ENDPROC
PROCEDURE Herramientas.cmdNuevo.Click
DODEFAULT()

Thisform.idcliente = 0
ENDPROC
PROCEDURE Herramientas.cmdEditar.Click
DODEFAULT()

Thisform.idcliente = cur_tempo.idcliente
ENDPROC
...
*** METODOS ***
PROCEDURE LostFocus
IF This.Parent.sel_tipoDoc.valcpoid = 1 .OR. This.Parent.sel_tipoDoc.valcpoid = 6 THEN
	IF !ValidarCUIT(This.Value) THEN
		MESSAGEBOX("El CUIT no es válido, por favor verifique el mismo y corrijalo", 0+48, Thisform.Caption)
	ENDIF
ENDIF
ENDPROC
...
Thisform.pgf.page2.cnt_fields.txtRazSoc.Value = loPersonaService.ObtenerRazonSocial()
Thisform.pgf.page2.cnt_fields.txtNomFant.Value = loPersonaService.ObtenerRazonSocial()
Thisform.pgf.page2.cnt_fields.txtDireccion.Value = loPersonaService.Direccion
Thisform.pgf.page2.cnt_fields.sel_Localid.txtCodigo.Value = loPersonaService.CodigoPostal
Thisform.pgf.page2.cnt_fields.sel_Localid.txtCodigo.LostFocus()
Thisform.pgf.page2.cnt_fields.sel_SitIVA.txtCodigo.Value = loPersonaService.ObtenerSitIVA()
Thisform.pgf.page2.cnt_fields.sel_SitIVA.txtCodigo.LostFocus()
Thisform.pgf.page2.cnt_fields.sel_tipodoc.txtCodigo.Value = "CUIT"
Thisform.pgf.page2.cnt_fields.sel_tipodoc.txtCodigo.LostFocus()
Thisform.pgf.page2.cnt_fields.sel_CondPago.txtCodigo.Value = 1
Thisform.pgf.page2.cnt_fields.sel_condPago.txtCodigo.LostFocus()
Thisform.pgf.page2.cnt_fields.sel_vendedor.txtCodigo.Value = 1
Thisform.pgf.page2.cnt_fields.sel_vendedor.txtCodigo.LostFocus()
Thisform.pgf.page2.cnt_fields.sel_TipoCli.txtCodigo.Value = 1
Thisform.pgf.page2.cnt_fields.sel_tipoCli.txtCodigo.LostFocus()
Thisform.pgf.page2.cnt_fields.sel_ramo.txtCodigo.Value = 1
Thisform.pgf.page2.cnt_fields.sel_ramo.txtCodigo.LostFocus()
Thisform.pgf.page2.cnt_fields.sel_transporte.txtCodigo.Value = 1
Thisform.pgf.page2.cnt_fields.sel_transporte.txtCodigo.LostFocus()
Thisform.pgf.page2.cnt_fields.chkhabilitado.Value = 1
Thisform.pgf.page2.cnt_fields.txtTelefono.SetFocus()
...
cfieldname = celular
ischaracter = .T.
msgerror = Debe ingresar el celular
requerido = .F.
Name = "txtCelular"
...
Left = 106
TabIndex = 29
cfieldname = idTipoCli
msgerror = Debe ingresar el tipo de cliente
nombre_campo_codigo = idTipoCli
nombre_campo_desc = descripcio
nombre_tabla = tiposcli
pkfield = idTipoCli
esnumerico = .T.
requerido = .T.
Name = "sel_tipoCli"
...
Width = 156
Alignment = 0
Caption = "Controlar Morosidad"
Value = 1
TabIndex = 33
cfieldname = ctrMoro
Name = "btnControlaMorosidad"
...
Width = 170
Alignment = 0
Caption = "Controlar Crédito Máximo"
Value = 1
TabIndex = 34
cfieldname = contrCM
Name = "chkCtrlCM"
...
TabIndex = 30
cfieldname = idRamo
msgerror = Debe ingresar el ramo
...
IF tipodoc.tipodoc == "CUIT" .OR. tipodoc.tipodoc == "CUIL" THEN
	This.Parent.txtNroCuit.InputMask = "XX-XXXXXXXX-X"
ELSE
	This.Parent.txtNroCuit.InputMask = ""
ENDIF
ENDPROC
