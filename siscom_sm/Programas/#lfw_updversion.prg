	...
	TRY
		...
		IF internet() THEN
			...
			* Instalacion de componentes
			...
			WAIT WINDOW "Chequeando y/o actualizando base de datos..." NOWAIT			
...
PROCEDURE actualizar_bd
	...
	**
	crearTablaLogUpgrade() && Creo tabla para log de actualizaciones.
...
************************************************************
* Creo la tabla para saber que se actualizó y qué no.
************************************************************
PROCEDURE crearTablaLogUpgrade
	LOCAL lcSql
...
************************************************************
* Este procedimiento permite corregir los duplicados que
* se generaron en DER.
************************************************************
PROCEDURE corregirDuplicadosDER
	LOCAL lcSql
	
	IF verificarActualizado("ARTIC-DER") THEN
		* Si esta actualización ya se ejecutó entonces, corto
		* la ejecución de la rutina para que no vuelva a ejecutar
		* la corrección.
		RETURN
	ENDIF
	
	TEXT TO lcSql NOSHOW
		create temporary table tmp_deleteart
			SELECT
				codArtPV,
				codArt,
				MAX(idArticulo) AS maxIdArticulo,
				MIN(idArticulo) AS minIdArticulo,
				count(*) as repetidos
			from		
				articulos
			where
	...
	TEXT TO lcSql NOSHOW
		alter table tmp_deleteart add index i_minIdArticulo (minIdArticulo);
	ENDTEXT
	ejecutar_comando(lcSql)
	...
	TEXT TO lcSql NOSHOW
		...
		set
		 	...
		 	idHostBaja = 'MANTENIMIENTO'
		 where
		 	idArticulo not in (		
		 		select minIdArticulo
		 		from tmp_deleteart)
	ENDTEXT
	ejecutar_comando(lcSql)
	
	TEXT TO lcSql NOSHOW
		INSERT INTO log_upgrade (codigo, descripcio, procedim)
		VALUES ('ARTIC-DER', 'Corrección de duplicados DER', '#lfw_updversion::corregirDuplicadosDER()')
	ENDTEXT
	ejecutar_comando(lcSql)
ENDPROC

******************************************************
* Verifica si una actualización ya se ejecutó
******************************************************
FUNCTION verificarActualizado
	PARAMETERS tcCodigo
	LOCAL lcSql
	LOCAL loRes
	LOCAL llResult
	...
	lcSql = loRes.addParameter(lcSql, "xcod", ALLTRIM(tcCodigo), .T., .F.)
