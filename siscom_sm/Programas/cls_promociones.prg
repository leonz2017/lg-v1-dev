...
DEFINE CLASS cls_promociones AS Custom
	...
	***********************************************************************
	PROCEDURE ExportarMerchandisingAJSON
		...
		STRTOFILE(lcJSON, getGlobalCFG("TEMPDIR") + "promo_merchandisign_cliente.json")
		loRes.Close_Query()
	ENDPROC
	...
	***********************************************************************
	PROCEDURE SubirArchivosJSON
		LOCAL loFTP
		LOCAL llResultConnection
		LOCAL lcFtpHost
		LOCAL lcFtpUser
		LOCAL lcFtpPassword
		LOCAL lcFtpPort
		LOCAL lcDirectorioInicial
		...
		llResultConnection = loFTP.OpenInternet(lcFtpUser, lcFtpPassword, lcFtpHost, lcFtpPort)
		IF !llResultConnection THEN
			MESSAGEBOX("Error al establecer la conexión FTP", 0+32, "Error de conexión")
		ENDIF
		
*!*			IF !loFTP.GetFtpDirectory(@lcDirectorioInicial) THEN
*!*				MESSAGEBOX(loFTP.GetErrorCode(.T.), 0+48, "Error de FTP - Obtener directorio")
*!*				lcDirectorioInicial = ""
*!*			ENDIF
		
		* Me paso al directorio donde tengo que subir los archivos
		IF !loFTP.ChangeFtpDirectory(getGlobalCFG("FTPINIDIR"))
			MESSAGEBOX(loFTP.GetErrorText(loFTP.GetErrorCode(.T.)), 0+48, "Error de FTP")
		ENDIF

		* Subo el archivo promocli.json				
		IF !loFTP.PutFtpFile("promocli.json", getGlobalCFG("TEMPDIR") + "promocli.json") THEN
			MESSAGEBOX(loFTP.GetErrorText(loFTP.GetErrorCode(.T.)), 0+48, "Error de FTP")
		ENDIF
		
		* Subo el archivo promo_merchandising_cliente.json
		IF !loFTP.PutFtpFile("promo_merchandising_cliente.json", getGlobalCFG("TEMPDIR") + "promo_merchandisign_cliente.json") THEN
			MESSAGEBOX(loFTP.GetErrorText(loFTP.GetErrorCode(.T.)), 0+48, "Error de FTP")
		ENDIF
	...
	******************************************************************************
	PROCEDURE SubirArchivosJSONPorSFTP
		...
		* Armo el comando a ejecutar mediante una macro de sustitución
		...
		lcCmd = STRTRAN(lcCmd, "nombre_archivo_2", getGlobalCFG("TEMPDIR") + "promo_merchandisign_cliente.json")
