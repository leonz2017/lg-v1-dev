...
IF ((ALLTRIM(cbte) == "NC") .OR. (ALLTRIM(cbte) == "FC") .OR. (ALLTRIM(cbte) == "PTO") .OR. (ALLTRIM(cbte) == "COT")) THEN
	...
	DO WHILE !EOF()
		...
		REPLACE cur_aux.detobs WITH IIF(ISNULL(cur_tempo.observ), "", ALLTRIM(cur_tempo.observ)) ADDITIVE
...
IF ALLTRIM(cbte) == "PED" THEN
	...
	DO WHILE !EOF()
		...
		REPLACE cur_detalle.detobs WITH IIF(ISNULL(cur_tempo.observ), "", ALLTRIM(cur_tempo.observ)) ADDITIVE
...
LOCAL lcDia, lcMes, lcAnio
LOCAL m.CodTrans, m.RazSocTR, m.qr, m.motivo
...
lcSql = "select ventascab.*, transp.codTrans, transp.razSoc from ventascab "
lcSql = lcSql + "	left join transp on ventascab.idTransp = transp.idTransp "
lcSql = lcsql + "where idventasc = " + ALLTRIM(STR(cur_cbtes.idventasc))
...
*!* ENDIF
if isnull(cur_venta.aut_qr) then
	if int(val(gnDEMO)) = 0 then
		m.qr = getglobalcfg("QRPATH") + "sin_qr.png"
	else
		m.qr = ""
...
ELSE
* Verifico si el archivo existe. Si no existe pongo la imagen sin_qr.png.
	IF !FILE(ALLTRIM(cur_venta.aut_qr)) then
		IF !Thisform.generar_qr(cur_venta.ptovta, int(val(cur_venta.aut_tipocbte)),ALLTRIM(STR(cur_venta.numcbte))) THEN 	
			RETURN .F.
		ENDIF 
		...
		lcSql = lcSql + "SET aut_qr = '" + alltrim(strtran(thisform.qr_image, "\", "\\")) + "' "
		lcSql = lcSql + "where idVentasC = " + ALLTRIM(STR(cur_cbtes.idventasc))
...
IF ISNULL(cur_venta.codTrans) THEN
	m.CodTrans = 0
	m.RazSocTR = ""
ELSE
	m.CodTrans = IIF(ISNULL(cur_venta.codTrans), "", cur_venta.codTrans)
	m.RazSocTR = ALLTRIM(IIF(ISNULL(cur_venta.razSoc), "", cur_venta.razSoc))
ENDIF
