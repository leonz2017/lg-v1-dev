...
************************************************************
OBJETO: FRMABMFAMILIAS
************************************************************
*** PROPIEDADES ***
DoCreate = .T.
Caption = "ABM Familias"
obj_dataservicename = datObject
pk_fieldname = idFamilia
cnombretabla = familias
titulos_cabeceras = Código,Descripción
listctrlsource = idFamilia,descripcio
...
lcSql = "SELECT COUNT(*) AS cant FROM articulos WHERE idFamilia = " + ALLTRIM(STR(cur_Tempo.idFamilia))
...
SELECT cur_x
IF cur_x.cant > 0
	MESSAGEBOX("La familia no puede ser eliminada ya que tiene artículos vinculados", 0+64, Thisform.Caption)
