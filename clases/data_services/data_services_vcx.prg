...
************************************************************
OBJETO: cl_dat_articulos
************************************************************
*** PROPIEDADES ***
Height = 17
Width = 109
Name = "cl_dat_articulos"
...
rs.ActiveConnection = goConn.ActiveConnection
rs.OpenQuery("CALL articulos_getAll()") 
...
************************************************************
OBJETO: cl_dat_art_exivapto
************************************************************
*** PROPIEDADES ***
idarticulo = 0
habilitado = 0
id = 0
Name = "cl_dat_art_exivapto"
...
loCmd.CommandText = lcSql
loCmd.AddParameter("idArticulo", ALLTRIM(STR(this.idarticulo)), .F., .F.)
loCmd.AddParameter("habilitado", ALLTRIM(STR(this.habilitado)), .F., .F.)
loCmd.AddParameter("usualta", ALLTRIM(gcCodUsu), .T., .F.)
loCmd.AddParameter("idHostAlta", ALLTRIM(SYS(0)), .T., .F.)

IF !loCmd.Execute() THEN
	MESSAGEBOX(loCmd.ErrorMessage, 0+48, "Alta de exceptuado")
	RETURN .F.
ENDIF

** Obtengo el último ID insertado.
lcSql = "SELECT MAX(id) AS id FROM art_exivapto"
loRes = CREATEOBJECT("odbc_result")
loRes.ActiveConnection = goConn.ActiveConnection
...
PROCEDURE getall
********************************************************************
* Obtiene todos los registros de la tabla art_exivapto.
********************************************************************
...
TEXT TO lcSql NOSHOW
	...
	FROM
		art_exivapto
			INNER JOIN articulos ON art_exivapto.idArticulo = articulos.idArticulo
	WHERE
		art_exivapto.fecBaja IS NULL
ENDTEXT
...
TEXT TO lcSql NOSHOW
	...
	SET
		...
		idHostModi = ?idHostModi
	WHERE
		art_exivapto.id = ?idRegistro
ENDTEXT

loCmd.CommandText = lcSql
loCmd.AddParameter("idArticulo", ALLTRIM(STR(this.idarticulo)), .F., .F.)
loCmd.AddParameter("habilitado", ALLTRIM(STR(this.habilitado)), .F., .F.)
loCmd.AddParameter("usumodi", ALLTRIM(gcCodUsu), .T., .F.)
loCmd.AddParameter("idHostModi", ALLTRIM(SYS(0)), .T., .F.)
loCmd.AddParameter("idRegistro", ALLTRIM(STR(this.id)), .F., .F.)
...
lcSql = "select	numActual, fecVto, impresora, cantCpia, repname, repnamecp "
...
this.repname = cur_tempo.repname
this.repnamecp = cur_tempo.repnamecp
...
lcSql = "select	tipodoc, ptovta, numActual, fecVto, impresora, cantCpia, repname, repnamecp "
lcSql = lcSql + "from numerador "
lcSql = lcSql + "where cbte = '" + ALLTRIM(this.cbte) + "' "
lcSql = lcSql + "	and tipoDoc = '" + ALLTRIM(this.tipodoc) + "' "
...
CREATE CURSOR cur_Detalle (	;
	...
	retCompro int,;
	retFecha datetime,;
	nroTransf varchar(20))
	
CREATE CURSOR cur_facasel ( ;
...
DO WHILE !EOF("cur_detalle")
	IF (cur_detalle.tipoPago == "CHEQ")
		IF (lnIdBanco = cur_detalle.idBanco) .AND. (ALLTRIM(lcchq_nro) == ALLTRIM(cur_detalle.chq_nro))
...
IF lcCbte == "RC" THEN
	...
	IF getGlobalCFG("NRC_AUTO") THEN
		** Pasa por acá si el número se carga automáticamente.
		loNum.cbte = lcCbte 
		...
		lbEsNumerado = .T.
	ELSE 
		** Pasa por acá si el número se carga de forma manual
		lcTipo = "X"
		loNum.cbte = lcCbte 
		loNum.tipoDoc = lcTipo 
	...
	ENDIF
ELSE
	* Pasa por acá si es movimiento de caja (4/10/2022)
	lcTipo = ""
	loNum.cbte = this.Cbte
	loNum.tipoDoc = lcTipo
	IF !loNum.getptovta() THEN
		goConn.Rollback()
		RETURN .F.
	ENDIF
...
this.repname = IIF(ISNULL(loNum.repname), "", loNum.repname)
this.repnamecp = IIF(ISNULL(loNum.repnamecp), "", loNum.repnamecp)		
...
IF this.imp_efec <> 0 THEN
	...
	loResult.OpenQuery("SELECT * FROM planctas WHERE codAbr = 'FPCJP'")
	...
	REPLACE cur_Detalle.descripcio WITH "EFECTIVO" ADDITIVE
...
IF This.imp_td <> 0 THEN
	...
	REPLACE cur_Detalle.descripcio WITH "TARJETA DE DEBITO" ADDITIVE
...
IF This.imp_tc <> 0 THEN
	...
	REPLACE cur_Detalle.descripcio WITH "TARJETA DE CREDITO" ADDITIVE
...
IF This.imp_iva <> 0 THEN
	...
	REPLACE cur_Detalle.descripcio WITH "RETENCION IVA" ADDITIVE
...
IF This.imp_ib <> 0 THEN
	...
	REPLACE cur_Detalle.descripcio WITH "RETENCION IIBB" ADDITIVE
...
IF This.imp_gan <> 0 THEN
	...
	REPLACE cur_Detalle.descripcio WITH "RETENCION GANANCIA" ADDITIVE
...
IF This.imp_suss <> 0 THEN
	...
	REPLACE cur_Detalle.descripcio WITH "RET. SUSS" ADDITIVE
...
IF lcCbte = "RC" THEN 
	...
	************************************************************************
	...
	DO WHILE !EOF("cur_detalle")
		...
		lcSql = "INSERT INTO rccob_d (idRCCob_D, idRCCob_C, idCheque, idPlanCta, idBanco, chq_nro, importe, tipoPago, idProvin, retCompro, retFecha, nroTransf) "
		...
		lcSql = lcSql + IIF(lnRetCbte = 0, "NULL", ALLTRIM(STR(lnRetCbte))) + ", " + IIF(ldRetFecha = {}, "NULL", loDT.tomysql(ldRetFecha)) + ", '" + ALLTRIM(cur_Detalle.nroTransf) + "')"
...
DO WHILE !EOF("cur_facturas")
	IF cur_facturas.selected THEN
		...
		lnIdCC_Orig = cur_facturas.idCC_Cli
		...
		IF ALLTRIM(cur_facturas.cbte) == "AD" THEN
			lcSql = "SELECT * FROM cc_cli WHERE idCC_Cli = " + ALLTRIM(STR(lnIdCC_Orig))
		ELSE
			lcSql = "SELECT * FROM cc_cli WHERE idVentasC = " + ALLTRIM(STR(cur_facturas.idVentasC))
		ENDIF
		loResult.ActiveConnection = goConn.ActiveConnection
		...
		lcSql = lcSql + " WHERE idCC_Cli = " + ALLTRIM(STR(lnIdCC_Orig))
		loCommand.ActiveConnection = goConn.ActiveConnection
		loCommand.CommandText = lcSql
		...
		IF lcCbte == "RC" THEN 
			...
			lcSql = lcSql + "VALUES (" + ALLTRIM(STR(lnIdRCCob_DV)) + ", " + ALLTRIM(STR(lnProxID)) + ", " ; 
				+ IIF(ISNULL(cur_facturas.idVentasC), "NULL", ALLTRIM(STR(cur_facturas.idVentasC))) + ", "
...
* Valido que haya artículos para generar movimientos y evitar que se genere solo la cabecera
* en caso de que stk_modulo = true y llevaStock = false
SELECT cur_stkdet
IF RECCOUNT("cur_stkdet") = 0 THEN
	This.ErrorMessage = "Este comprobante no tiene artículos habilitados para llevar stock"
	RETURN .T.
ENDIF
...
LOCAL lnIdStkDep, lcCodArt, lcDescripcio,lnLlevaStk
...
IF getglobalcfg("stk_module")then
	IF !lnLlevaStk THEN
		This.errorMessage = "El comprobante tiene artículos que no están habilitados para llevar stock"
		RETURN .F.
	ENDIF
ENDIF 
...
ELSE
*!*		lcSql = lcSql + "WHERE CAST(stk_cab.fecAlta AS DATE) BETWEEN " + loDT.toMySql(tdFecDD) + " AND " + loDT.toMySql(tdFecHH) + " "
	lcSql = lcSql + "WHERE stk_cab.fecAlta IS NOT NULL AND articulos.fecBaja is null "
ENDIF
...
ENDPROC
PROCEDURE lleva_stock
**********************************************************
* Verifica si un artículo lleva o no stock
**********************************************************
PARAMETERS tnIdArticulo
...
lcSql = loRes.AddParameter(lcSql, "xidArticulo", ALLTRIM(STR(tnIdArticulo)), .f., .f.)
