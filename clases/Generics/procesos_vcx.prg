...
loResult = CREATEOBJECT("odbc_result")
lcSql = "CALL arti_actupreGetByAny( "	;
	+ "?pIdProvDD, ?pIdProvHH, " 			;
	+ "?pÌdFamiliaDD, ?pIdFamiliaHH, "	;
	+ "?pIdSubFamDD, ?pIdSubFamHH, " 		;
	+ "?pIdMarcaDD, ?pIdMarcaHH, " 		;
	+ "?pCodArtDD, ?pCodArtHH)"
lcSql = loResult.AddParameter(lcSql, "pIdProvDD", ALLTRIM(STR(this.idprov_dd)), .f., .f.)
lcSql = loResult.AddParameter(lcSql, "pIdProvHH", ALLTRIM(STR(this.idprov_hh)), .f., .f.)
lcSql = loResult.AddParameter(lcSql, "pÌdFamiliaDD", ALLTRIM(STR(this.idfamilia_dd)), .f., .f.)
lcSql = loResult.AddParameter(lcSql, "pIdFamiliaHH", ALLTRIM(STR(this.idfamilia_hh)), .f., .f.)
lcSql = loResult.AddParameter(lcSql, "pIdSubFamDD", ALLTRIM(STR(this.idsubfam_dd)), .f., .f.)
lcSql = loResult.AddParameter(lcSql, "pIdSubFamHH", ALLTRIM(STR(this.idsubfam_hh)), .f., .f.)
lcSql = loResult.AddParameter(lcSql, "pIdMarcaDD", ALLTRIM(STR(this.idmarca_dd)), .f., .f.)
lcSql = loResult.AddParameter(lcSql, "pIdMarcaHH", ALLTRIM(STR(this.idmarca_hh)), .f., .f.)
lcSql = loResult.AddParameter(lcSql, "pCodArtDD", ALLTRIM(this.codart_dd), .t., .f.)
lcSql = loResult.AddParameter(lcSql, "pCodArtHH", ALLTRIM(this.codart_hh), .t., .f.)
loResult.ActiveConnection = goConn.ActiveConnection
loResult.Cursor_Name = "cur_tmp"
IF !loResult.OpenQuery(lcSql) THEN
	this.ErrorMessage = loResult.Error_Message
...
loRes = CREATEOBJECT("odbc_result")
lcSql = "CALL upd_ArtNuevosProcesar (?idProv)"
lcSql = loRes.AddParameter(lcSql, "idProv", ALLTRIM(STR(Thisform.contenido.sel_proveedor.valcpoid)), .f., .f.)
loRes.ActiveConnection = goConn.ActiveConnection
...
IF !thisform.o_updater.copiar_planilla_a_cursor("") THEN
	MESSAGEBOX(thisform.o_updater.mensaje, 0+16, Thisform.Caption)
	RETURN
ENDIF
...
DO WHILE !EOF("cur_rubros")
	...
	IF RECCOUNT("cur_subfam") = 0 THEN
		...
		lcSql = "INSERT INTO subfam ( idSubFam, descripcio, usuAlta, fecAlta, idHostAlta ) VALUES ("
		lcSql = lcSql + ALLTRIM(STR(lnIdSubFam)) + ", "
		lcSql = lcSql + "'" + ALLTRIM(cur_rubros.Titulo) + "', "
		lcSql = lcSql + "'" + ALLTRIM(gcCodUsu) + "', "
		lcSql = lcSql + loDT.getDateTime() + ", "
		lcSql = lcSql + "'" + ALLTRIM(SYS(0)) + "')"
	...
	DO WHILE !EOF("cur_arti")
		...
		IF RECCOUNT("cur_tmp") = 0 THEN
			...
			loCommand.AddParameter("descripcio", ALLTRIM(STRTRAN(cur_arti.Descrip, "'"," ")), .t., .f.)
			...
			loCommand.AddParameter("cdaProv", ALLTRIM(lcCodigo), .t., .f.)
...
DO WHILE !EOF("cur_articulos")
	...
	IF lnPrLista <> 0 THEN
		...
		lcSql = "CALL articulos_actuPreCatalogo ( " ;
			...
			+ "?pCodUsu, " ;
			+ "?pIdHostModi) "
		lcSql = loCommand.AddParameter(lcSql, "pCodArt", ALLTRIM(cur_articulos.codArt), .t., .f.)
...
=This.detectar_codigos_eliminar()
...
ENDPROC
PROCEDURE detectar_codigos_eliminar
* *********************************************************************
* Este método permite detectar los códigos que no figuran más en el
* catálogo.
* Fecha: 14/07/2021
...
WAIT WINDOW "Detectando artículos a eliminar" NOWAIT

&& Levanto los artículos desde el sistema
loRes = CREATEOBJECT("odbc_result")
...
&& Levanto los artículos desde el catálogo
SELECT * FROM C:\TOMMASIW\arti ;
INTO CURSOR cur_tmssi

&& Busco los artículos que no figuran cruzando los dos cursores
SELECT 																		;
	cur_articulos.idarticulo,												;
	cur_articulos.codArt,													;
	cur_articulos.codArtPV,													;
	cur_articulos.descripcio												;
FROM																		;
	cur_articulos															;
		LEFT OUTER JOIN cur_tmssi 											;
			ON ALLTRIM(cur_tmssi.codigo) == ALLTRIM(cur_articulos.codArtPV) ;
WHERE																		;
	cur_tmssi.codigo IS NULL 												;
INTO CURSOR cur_eliminar

SELECT cur_eliminar
IF RECCOUNT("cur_eliminar") > 0 THEN
	MESSAGEBOX("Se detectaron " + ALLTRIM(STR(RECCOUNT("cur_eliminar"))) ;
			+ " articulos a eliminar", 0+64, "Actualización desde catálogo")

	MESSAGEBOX("A continuación se deberá grabar la planilla con los posibles "			;
			+ "artículos a eliminar. Recomiendo revisar bien antes de dar de baja " 	;
			+ "los mismos.", 0+64, "Actualización desde catálogo")

	&& Guardo el excel con los artículos detectados
	lcFileName = GETFILE(			;
			"xls", 					;
			"Nombre de archivo", 	;
			"Guardar", 				;
			1, 						;
			"Planilla con artículos a eliminar")
...
loCmd.ActiveConnection = goConn.ActiveConnection
loCmd.CommandText = "CALL artcat_CrearTemps (?pIdProv)"
...
DO WHILE !EOF("cur_artic")
	...
	ENDTEXT
	lcSql = loRes.AddParameter(lcSql, "pIdProv", ALLTRIM(STR(This.idprov)), .f., .f.)
	lcSql = loRes.AddParameter(lcSql, "pCodArt", ALLTRIM(cur_artic.codigo), .t., .f.)
	lcSql = loRes.AddParameter(lcSql, "pDescripcio", ALLTRIM(cur_artic.descripcio), .t., .f.)
	lcSql = loRes.AddParameter(lcSql, "pFamilia", ALLTRIM(cur_artic.familia), .t., .f.)
	lcSql = loRes.AddParameter(lcSql, "pSubfam", ALLTRIM(cur_artic.subfam), .t., .f.)
	lcSql = loRes.AddParameter(lcSql, "pMarca", ALLTRIM(cur_artic.marca), .t., .f.)
	lcSql = loRes.AddParameter(lcSql, "pPrLista", ALLTRIM(STR(cur_artic.precio, 10, 2)), .f., .f.)
	lcSql = loRes.AddParameter(lcSql, "pBonif1", ALLTRIM(STR(this.desc1)), .f., .f.)
	lcSql = loRes.AddParameter(lcSql, "pBonif2", ALLTRIM(STR(this.desc2)), .f., .f.)
	lcSql = loRes.AddParameter(lcSql, "pBonif3", ALLTRIM(STR(this.desc3)), .f., .f.)
	lcSql = loRes.AddParameter(lcSql, "pBonif4", ALLTRIM(STR(this.desc4)), .f., .f.)
	lcSql = loRes.AddParameter(lcSql, "pMargenMay", ALLTRIM(STR(this.margenmay)), .f., .f.)
	lcSql = loRes.AddParameter(lcSql, "pMargenMin", ALLTRIM(STR(this.margenmin)), .f., .f.)
	...
	ELSE
		IF !(ALLTRIM(cur_x.result) == "OK") THEN
			brow
			MESSAGEBOX("Codigo: " + ALLTRIM(cur_artic.codigo) , 0+48, Thisform.Caption)
			loRes.Close_Query()
			RETURN .F.
		ENDIF
		loRes.Close_Query()
	...
	i = i + 1
	WAIT WINDOW "Leyendo " + ALLTRIM(STR(i)) + " de " + ALLTRIM(STR(RECCOUNT("cur_artic"))) NOWAIT
...
WAIT WINDOW "Actualizando precios..." NOWAIT
loRes = CREATEOBJECT("odbc_result")
lcSql = "CALL artcat_update (?pUsuario, ?pHostName)"
lcSql = loRes.AddParameter(lcSql, "pUsuario", ALLTRIM(gcCodUsu), .t., .f.)
lcSql = loRes.AddParameter(lcSql, "pHostName", ALLTRIM(SYS(0)), .t., .f.)
loRes.ActiveConnection = goConn.ActiveConnection
loRes.Cursor_Name = "cur_x"
IF !loRes.OpenQuery(lcSql) THEN
	MESSAGEBOX("Error al ejecutar " + lcSql, 0+48, Thisform.Caption)
	RETURN .F.
ENDIF
SELECT cur_x
IF !(ALLTRIM(cur_x.result) == "OK") THEN
...
lcSql = "CALL artcat_insert (?pIdProv, ?pBonif1, ?pBonif2, ?pBonif3, ?pBonif4, ?pMargenMay, ?pMargenMin, ?pUsuario, ?pHostName)"
lcSql = loRes.AddParameter(lcSql, "pIdProv", ALLTRIM(STR(this.idprov)), .f., .f.)
lcSql = loRes.AddParameter(lcSql, "pBonif1", ALLTRIM(STR(this.desc1, 10, 2)), .f., .f.)
lcSql = loRes.AddParameter(lcSql, "pBonif2", ALLTRIM(STR(this.desc2, 10, 2)), .f., .f.)
lcSql = loRes.AddParameter(lcSql, "pBonif3", ALLTRIM(STR(this.desc3, 10, 2)), .f., .f.)
lcSql = loRes.AddParameter(lcSql, "pBonif4", ALLTRIM(STR(this.desc4, 10, 2)), .f., .f.)
lcSql = loRes.AddParameter(lcSql, "pMargenMay", ALLTRIM(STR(this.margenmay, 10, 2)), .f., .f.)
lcSql = loRes.AddParameter(lcSql, "pMargenMin", ALLTRIM(STR(this.margenmin, 10, 2)), .f., .f.)
lcSql = loRes.AddParameter(lcSql, "pUsuario", ALLTRIM(gcCodUsu), .t., .f.)
lcSql = loRes.AddParameter(lcSql, "pHostName", ALLTRIM(SYS(0)), .t. , .f.)
loRes.ActiveConnection = goConn.ActiveConnection
...
IF !loRes.OpenQuery(lcSql) THEN
	MESSAGEBOX("Error al ejecutar " + lcSql, 0+48, "Actualización desde catálogo")
	RETURN .F.
ENDIF
IF !(ALLTRIM(cur_result.result) == "OK") THEN
	BROWSE
	MESSAGEBOX(ALLTRIM(cur_result.result), 0+48, "Actualización desde catálogo")
	loRes.Close_Query()
	RETURN .F.
ENDIF
loRes.Close_Query()
...
loDT = CREATEOBJECT("datetime")
loCommand = CREATEOBJECT("odbc_command")
loCmdUpdate = CREATEOBJECT("odbc_result")
loResult = CREATEOBJECT("odbc_result")
loUniDesp = CREATEOBJECT("odbc_result")
lcSql = ""
lnCostoRep = 0.00
...
IF RECCOUNT("cur_tmp") = 0 THEN
	...
	* Calculo el precio de venta
	...
	ENDTEXT
	loCommand.AddParameter("idProv", ALLTRIM(STR(This.idProv)), .f., .f.)
	loCommand.AddParameter("idFamilia", ALLTRIM(STR(tnIdFamilia)), .f., .f.)
	loCommand.AddParameter("idSubFam", ALLTRIM(STR(tnIdSubFam)), .f., .f.)
	loCommand.AddParameter("idMarca", ALLTRIM(STR(tnIdMarca)), .f., .f.)
	loCommand.AddParameter("codArt", ALLTRIM(lcCodigo), .t., .f.)
	loCommand.AddParameter("descripcio", ALLTRIM(STRTRAN(tcDescripcio, "'"," ")), .t., .f.)
	loCommand.AddParameter("alicIVA", ALLTRIM(STR(This.alic_iva, 10, 2)), .f., .f.)
	loCommand.AddParameter("tMon", "PSO", .t., .f.)
	loCommand.AddParameter("prListaExt", "0", .f., .f.)
	loCommand.AddParameter("cotizac", "0", .f., .f.)
	...
	loCommand.AddParameter("bonif4", ALLTRIM(STR(this.desc4, 10, 2)), .f., .f.)
	loCommand.AddParameter("costoRep", ALLTRIM(STR(lnCostoRep, 10, 2)), .f., .f.)
	loCommand.AddParameter("margenMax", ALLTRIM(STR(This.margenmay, 10, 2)), .f., .f.)
	loCommand.AddParameter("margenMin", ALLTRIM(STR(This.margenmin, 10, 2)), .f., .f.)
	loCommand.AddParameter("prVentaMax", ALLTRIM(STR(lnPrVentaMax, 10, 2)), .f., .f.)
	loCommand.AddParameter("prVentaMin", ALLTRIM(STR(lnPrVentaMin, 10, 2)), .f., .f.)
	loCommand.AddParameter("prFinalMax", ALLTRIM(STR(lnFinalMax, 10, 2)), .f., .f.)
	loCommand.AddParameter("prFinalMin", ALLTRIM(STR(lnFinalMin, 10, 2)), .f., .f.)
	loCommand.AddParameter("observ", "", .t., .f.)
	loCommand.AddParameter("habilitado", "1", .f., .f.)
	loCommand.AddParameter("linkFoto", "", .t., .f.)
	loCommand.AddParameter("usuAlta", ALLTRIM(gcCodUsu), .t., .f.)
...
ELSE
	...
	* Calculo el precio de venta
	...
	lcSql = "CALL articulos_actuPreCatalogo ( " ;
		...
		+ "?pCodUsu, " ;
		+ "?pIdHostModi) "
	lcSql = loCmdUpdate.AddParameter(lcSql, "pCodArt", ALLTRIM(lcCodigo), .t., .f.)
	lcSql = loCmdUpdate.AddParameter(lcSql, "pIdProv", ALLTRIM(STR(This.idprov)), .f., .f.)
	lcSql = loCmdUpdate.AddParameter(lcSql, "pPrLista", ALLTRIM(STR(lnPrLista, 10, 2)), .f., .f.)
	lcSql = loCmdUpdate.AddParameter(lcSql, "pBonif1", ALLTRIM(STR(lnBonif1, 10, 2)), .f., .f.)
	lcSql = loCmdUpdate.AddParameter(lcSql, "pBonif2", ALLTRIM(STR(lnBonif2, 10, 2)), .f., .f.)
	lcSql = loCmdUpdate.AddParameter(lcSql, "pBonif3", ALLTRIM(STR(lnBonif3, 10, 2)), .f., .f.)
	lcSql = loCmdUpdate.AddParameter(lcSql, "pBonif4", ALLTRIM(STR(lnBonif4, 10, 2)), .f., .f.)
	lcSql = loCmdUpdate.AddParameter(lcSql, "pCostoRep", ALLTRIM(STR(lnCostoRep, 10, 2)), .f., .f.)
	lcSql = loCmdUpdate.AddParameter(lcSql, "pMargenMax", ALLTRIM(STR(lnMargenMax, 10, 2)), .f., .f.)
	lcSql = loCmdUpdate.AddParameter(lcSql, "pMargenMin", ALLTRIM(STR(lnMargenMin, 10, 2)), .f., .f.)
	lcSql = loCmdUpdate.AddParameter(lcSql, "pPrVentaMax", ALLTRIM(STR(lnPrVentaMax, 10, 2)), .f., .f.)
	lcSql = loCmdUpdate.AddParameter(lcSql, "pPrVentaMin", ALLTRIM(STR(lnPrVentaMin, 10, 2)), .f., .f.)
	lcSql = loCmdUpdate.AddParameter(lcSql, "pPrFinalMax", ALLTRIM(STR(lnFinalMax, 10, 2)), .f., .f.)
	lcSql = loCmdUpdate.AddParameter(lcSql, "pPrFinalMin", ALLTRIM(STR(lnFinalMin, 10, 2)), .f., .f.)
	lcSql = loCmdUpdate.AddParameter(lcSql, "pCodUsu", ALLTRIM(gcCodUsu), .t., .f.)
	lcSql = loCmdUpdate.AddParameter(lcSql, "pIdHostModi", ALLTRIM(SYS(0)), .t., .f.)
	
	loCmdUpdate.ActiveConnection = goConn.ActiveConnection
	loCmdUpdate.Cursor_Name = "cur_res"
	IF !loCmdUpdate.OpenQuery(lcSql) THEN
		this.error_message = loCmdUpdate.Error_Message
		RETURN .F.
	ELSE
		IF !(ALLTRIM(cur_res.result) == "OK") THEN
			this.error_message = cur_res.result
			loCmdUpdate.Close_Query()
			RETURN .F.
		ENDIF
		loCmdUpdate.Close_Query()
	ENDIF
...
loRes.OpenQuery(lcSql)
SELECT resultado
llOk = IIF(ALLTRIM(resultado.result) == "OK", .T., .F.)
loRes.Close_Query()
IF !llOk THEN
	this.mensaje = "Error al intentar limpiar los datos temporales"
	RETURN .F.
...
DO WHILE !FEOF(lnFile)
	WAIT WINDOW "Procesando línea nº " + ALLTRIM(STR(lnIndex)) NOWAIT
	lcLinea = FGETS(lnFile)
	lcCodArt = ALLTRIM(GETWORDNUM(lcLinea, 1, "|"))
	lcCodigo = ALLTRIM(GETWORDNUM(lcLinea, 2, "|"))
	lcDescripcion = SUBSTR(ALLTRIM(GETWORDNUM(lcLinea, 3, "|")), 1, 200)
	lcDescripcion = IIF(ALLTRIM(lcDescripcion) == "", "SIN DESCRIPCION", lcDescripcion)
	lnLista = VAL(GETWORDNUM(lcLinea, 4, "|"))
	lnBonif1 = VAL(GETWORDNUM(lcLinea, 5, "|"))
	lnBonif2 = VAL(GETWORDNUM(lcLinea, 6, "|"))
	lnBonif3 = VAL(GETWORDNUM(lcLinea, 7, "|"))
	lnBonif4 = VAL(GETWORDNUM(lcLinea, 8, "|"))
	lnMargenMay = VAL(GETWORDNUM(lcLinea, 9, "|"))
	lnMargenMin = VAL(GETWORDNUM(lcLinea, 10, "|"))

	lcSql = "call art_upd_tempo_insert ( " ;
		+ "?pIdArticulo, ?pIdProv, ?pStock, ?pCodigo, " ;
		+ "?pCodArt, ?pArticulo, ?pLinea, ?pRubro, ?pVenta, ?pLista, ?pCosto, " ;
		+ "?pBonif1, ?pBonif2, ?pBonif3, ?pBonif4, ?pMargenMay, ?pMargenMin)"
	...
	lcSql = loRes.AddParameter(lcSql, "pCodArt", ALLTRIM(lcCodArt), .t., .f.)
	...
	lcSql = loRes.AddParameter(lcSql, "pBonif1", ALLTRIM(STR(lnBonif1, 3, 2)), .f., .f.)
	lcSql = loRes.AddParameter(lcSql, "pBonif2", ALLTRIM(STR(lnBonif2, 3, 2)), .f., .f.)
	lcSql = loRes.AddParameter(lcSql, "pBonif3", ALLTRIM(STR(lnBonif3, 3, 2)), .f., .f.)
	lcSql = loRes.AddParameter(lcSql, "pBonif4", ALLTRIM(STR(lnBonif4, 3, 2)), .f., .f.)
	lcSql = loRes.AddParameter(lcSql, "pMargenMay", ALLTRIM(STR(lnMargenMay, 3, 2)), .f., .f.)
	lcSql = loRes.AddParameter(lcSql, "pMargenMin", ALLTRIM(STR(lnMargenMin, 3, 2)), .f., .f.)
	loRes.ActiveConnection = goConn.ActiveConnection
	loRes.Cursor_Name = "cur_res"
	loRes.OpenQuery(lcSql)
	llOk = IIF(cur_res.result == "OK", .T., .F.)
	IF !llOk THEN
		this.mensaje = "Error: " + ALLTRIM(cur_res.result) + " - Código: " + ALLTRIM(lcCodigo)
		loRes.Close_Query()
		RETURN .F.
	ENDIF
	loRes.Close_Query()
...
WAIT WINDOW "Procesando bonificaciones y márgenes..." NOWAIT
...
loRes.Cursor_Name = "cur_res"
loRes.OpenQuery(lcSql)
llOk = IIF(ALLTRIM(cur_res.result) = "OK", .T., .F.)
loRes.Close_Query()
IF !llOk THEN
	this.mensaje = "Error al procesar los registros temporales"
	RETURN .F.
...
contenido.Clsetiqueta1.TabIndex = 18
...
contenido.Clsetiqueta2.TabIndex = 19
contenido.Clsetiqueta2.Name = "Clsetiqueta2"
contenido.txtFileName.TabIndex = 2
contenido.txtFileName.Name = "txtFileName"
contenido.btnCerrar.TabIndex = 10
contenido.btnCerrar.Name = "btnCerrar"
contenido.Clsetiqueta3.Left = 517
contenido.Clsetiqueta3.Top = 73
contenido.Clsetiqueta3.Visible = .F.
contenido.Clsetiqueta3.TabIndex = 20
contenido.Clsetiqueta3.Name = "Clsetiqueta3"
contenido.txtBonif1.Left = 604
contenido.txtBonif1.TabIndex = 11
contenido.txtBonif1.Top = 70
contenido.txtBonif1.Visible = .F.
contenido.txtBonif1.Name = "txtBonif1"
contenido.txtBonif2.Left = 676
contenido.txtBonif2.TabIndex = 12
contenido.txtBonif2.Top = 70
contenido.txtBonif2.Visible = .F.
contenido.txtBonif2.Name = "txtBonif2"
contenido.txtBonif3.Left = 748
contenido.txtBonif3.TabIndex = 13
contenido.txtBonif3.Top = 70
contenido.txtBonif3.Visible = .F.
contenido.txtBonif3.Name = "txtBonif3"
contenido.txtBonif4.Left = 820
contenido.txtBonif4.TabIndex = 14
contenido.txtBonif4.Top = 70
contenido.txtBonif4.Visible = .F.
contenido.txtBonif4.Name = "txtBonif4"
contenido.Clsetiqueta4.Left = 517
contenido.Clsetiqueta4.Top = 96
contenido.Clsetiqueta4.Visible = .F.
contenido.Clsetiqueta4.TabIndex = 21
contenido.Clsetiqueta4.Name = "Clsetiqueta4"
contenido.txtMargenMay.Left = 626
contenido.txtMargenMay.TabIndex = 15
contenido.txtMargenMay.Top = 93
contenido.txtMargenMay.Visible = .F.
contenido.txtMargenMay.Name = "txtMargenMay"
contenido.Clsetiqueta5.Left = 711
contenido.Clsetiqueta5.Top = 96
contenido.Clsetiqueta5.Visible = .F.
contenido.Clsetiqueta5.TabIndex = 22
contenido.Clsetiqueta5.Name = "Clsetiqueta5"
contenido.txtMargenMin.Left = 820
contenido.txtMargenMin.TabIndex = 16
contenido.txtMargenMin.Top = 93
...
contenido.grdRegistros.TabIndex = 17
contenido.grdRegistros.Name = "grdRegistros"
contenido.btnAceptar.Top = 73
contenido.btnAceptar.Left = 897
contenido.btnAceptar.TabIndex = 6
contenido.btnAceptar.Name = "btnAceptar"
contenido.btnGrabar.TabIndex = 8
contenido.btnGrabar.Name = "btnGrabar"
contenido.btnCancelar.TabIndex = 9
...
IF (ALLTRIM(Thisform.Contenido.txtFileName.Value) == "") .OR. (ALLTRIM(Thisform.Contenido.txtFileName.Value) == "X") THEN
...
DO WHILE !(ALLTRIM(lcCodArtPV) == "") .AND. !ISNULL(loSheet.cells(lnIndex, 2).Value)
	IF !Thisform.add_to_cur_xls(loSheet, lnIndex) THEN
		lnCantInvalidos = lnCantInvalidos + 1
	ENDIF
...
IF lnCantInvalidos > 0 THEN
	MESSAGEBOX("Se encontraron " + ALLTRIM(STR(lnCantInvalidos)) + " registros inválidos", 0+48, Thisform.Caption)
	RETURN .F.
ENDIF
...
RETURN .T.
ENDPROC
PROCEDURE add_registro_a_procesar
*
* Este método permite agregar el registro que deberá ser procesado.
*
...
REPLACE registros.bonif1 		WITH IIF(cur_xls.bonif1 < 0, cur_art.bonif1, cur_xls.bonif1) ADDITIVE
REPLACE registros.bonif2 		WITH IIF(cur_xls.bonif2 < 0, cur_art.bonif2, cur_xls.bonif2) ADDITIVE
REPLACE registros.bonif3 		WITH IIF(cur_xls.bonif3 < 0, cur_art.bonif3, cur_xls.bonif3) ADDITIVE
REPLACE registros.bonif4 		WITH IIF(cur_xls.bonif4 < 0, cur_art.bonif4, cur_xls.bonif4) ADDITIVE
REPLACE registros.margenMin 	WITH IIF(cur_xls.margenMin <= 0, cur_art.margenMin, cur_xls.margenMin) ADDITIVE
REPLACE registros.margenMay 	WITH IIF(cur_xls.margenMay <= 0, cur_art.margenMax, cur_xls.margenMay) ADDITIVE
REPLACE registros.prListaPV 	WITH cur_xls.precio ADDITIVE
REPLACE registros.prListaAnt 	WITH cur_art.prLista ADDITIVE

ENDPROC
PROCEDURE add_registro_nuevo
*
* Agrega un artículo que fué detectado como nuevo
*
...
ENDPROC
PROCEDURE add_registro_baja
*
* Agrega un artículo posible a dar de baja
*
...
ENDPROC
PROCEDURE buscar_articulos_nuevos
*
* Este método permite buscar los artículos nuevos.
*
...
USE IN cur_nuevos
ENDPROC
PROCEDURE buscar_posibles_bajas
*
* Este método permite buscar artículos con posibilidades a dar de baja.
*
LOCAL loRes
...
TEXT TO lcSql NOSHOW
	...
	FROM 
		articulos
	WHERE 
		articulos.idProv = ?idProv AND 
		articulos.fecBaja IS NULL AND
		CASE WHEN ?idMarca = 0 THEN 1 ELSE articulos.idMarca = ?idMarca END
ENDTEXT
lcSql = loRes.AddParameter(lcSql, "idProv", ALLTRIM(STR(Thisform.Contenido.sel_proveedor.valcpoid)), .f., .f.)
lcSql = loRes.AddParameter(lcSql, "idMarca", ALLTRIM(STR(Thisform.ContenidO.sel_marcas.valcpoid)), .f., .f.)
...
ENDPROC
PROCEDURE add_to_cur_xls
*
* Este método permite agregar la fila de la planilla a cur_xls
*

parameters toSheet, tnIndex

local lcCodArtPV

if alltrim(type("toSheet.cells(tnIndex, 1).value")) == "C" then
	lcCodArtPV = alltrim(toSheet.cells(tnIndex, 1).value)
else
	if alltrim(type("toSheet.cells(tnIndex, 1).value")) == "N" then
		lcCodArtPV = alltrim(str(toSheet.cells(tnIndex, 1).value))
	else
		messagebox("Verifique el código de artículo en la fila " + alltrim(str(tnIndex)) + " que el dato es incorrecto", 0+48, Thisform.Caption)
		return .f.
	endif
endif

if isnull(lcCodArtPV) then
	messagebox("Hay un error en el código en la fila " + alltrim(str(tnIndex)), 0+48, Thisform.Caption)
	return .f.
endif

if !(alltrim(type("toSheet.cells(tnIndex, 2).value")) == "N") then
	messagebox("Hay un error en el precio en la fila " + alltrim(str(tnIndex)), 0+48, Thisform.Caption)
	return .f.
endif

if isnull(toSheet.cells(tnIndex, 2).value) then
	return .f.
endif

if !(alltrim(type("toSheet.cells(tnIndex, 3).value")) == "N") then
	messagebox("Hay un error en la bonificación 1 en la fila " + alltrim(str(tnIndex)), 0+48, Thisform.Caption)
	return .f.
endif

if isnull(toSheet.cells(tnIndex, 3).value) then
	return .f.
endif

if !(alltrim(type("toSheet.cells(tnIndex, 4).value")) == "N") then
	messagebox("Hay un error en la bonificación 2 en la fila " + alltrim(str(tnIndex)), 0+48, Thisform.Caption)
	return .f.
endif

if isnull(toSheet.cells(tnIndex, 4).value) then
	return .f.
endif

if !(alltrim(type("toSheet.cells(tnIndex, 5).value")) == "N") then
	messagebox("Hay un error en la bonificación 3 en la fila " + alltrim(str(tnIndex)), 0+48, Thisform.Caption)
	return .f.
endif

if isnull(toSheet.cells(tnIndex, 5).value) then
	return .f.
endif

if !(alltrim(type("toSheet.cells(tnIndex, 6).value")) == "N") then
	messagebox("Hay un error en la bonificación 4 en la fila " + alltrim(str(tnIndex)), 0+48, Thisform.Caption)
	return .f.
endif

if isnull(toSheet.cells(tnIndex, 6).value) then
	return .f.
endif

if !(alltrim(type("toSheet.cells(tnIndex, 7).value")) == "N") then
	messagebox("Hay un error en margen mayorista en la fila " + alltrim(str(tnIndex)), 0+48, Thisform.Caption)
	return .f.
endif

if isnull(toSheet.cells(tnIndex, 7).value) then
	return .f.
endif

if !(alltrim(type("toSheet.cells(tnIndex, 8).value")) == "N") then
	messagebox("Hay un error en margen minorista en la fila " + alltrim(str(tnIndex)), 0+48, Thisform.Caption)
	return .f.
endif

if isnull(toSheet.cells(tnIndex, 8).value) then
	return .f.
endif

* Para tomar el codigo literalmente como texto uso el metodo "Text".

select cur_xls
append blank
replace cur_xls.codArtPV with alltrim(lcCodArtPV)
replace cur_xls.precio with toSheet.cells(tnIndex, 2).value additive
replace cur_xls.bonif1 with toSheet.cells(tnIndex, 3).value additive
replace cur_xls.bonif2 with toSheet.cells(tnIndex, 4).value additive
replace cur_xls.bonif3 with toSheet.cells(tnIndex, 5).value additive
replace cur_xls.bonif4 with toSheet.cells(tnIndex, 6).value additive
replace cur_xls.margenMay with toSheet.cells(tnIndex, 7).value additive
replace cur_xls.margenMin with toSheet.cells(tnIndex, 8).value additive
...
PROCEDURE blanquear
DODEFAULT()
Thisform.Contenido.sel_marcas.blanquear()
Thisform.Contenido.sel_marcas.valcpoid = 0
Thisform.Contenido.sel_marcas.txtCodigo.Enabled = .F.
Thisform.Contenido.chkFiltrarMarca.Value = 0
ENDPROC
PROCEDURE Init
DODEFAULT()
...
DO WHILE !EOF("cur_xls")
	TEXT TO lcSql NOSHOW
		...
		FROM 
			articulos
		WHERE 
			articulos.codArtPV = ?codArtPV AND 
			articulos.idProv = ?idProv AND
			CASE WHEN ?idMarca = 0 THEN 1 ELSE articulos.idMarca = ?idMarca END AND
			articulos.fecBaja IS NULL
	ENDTEXT
	lcSql = loRes.AddParameter(lcSql, "codArtPV", ALLTRIM(cur_xls.codArtPV), .t., .f.)
	lcSql = loRes.AddParameter(lcSql, "idProv", ALLTRIM(STR(Thisform.Contenido.sel_proveedor.valcpoid)), .f., .f.)
	lcSql = loRes.AddParameter(lcSql, "idMarca", ALLTRIM(STR(Thisform.Contenido.sel_marcas.valcpoid)), .f., .f.)
	loRes.ActiveConnection = goConn.ActiveConnection
	loRes.Cursor_Name = "cur_art"
	...
	ELSE
		IF (RECCOUNT("cur_art") = 1) THEN
			Thisform.add_registro_a_procesar()
		ENDIF
...
Thisform.buscar_articulos_nuevos()
Thisform.buscar_posibles_bajas()
...
CREATE CURSOR cur_artbaja ( ;
	...
	precio double)
ENDPROC
PROCEDURE contenido.sel_proveedor.recuperar_datos
* Corto cadena de herencia
ENDPROC
...
Caption = "Marca:"
TabIndex = 4
Name = "chkFiltrarMarca"

*** METODOS ***
PROCEDURE InteractiveChange
IF This.Value = 1 THEN
	Thisform.Contenido.sel_marcas.txtCodigo.Enabled = .T.
ELSE
	Thisform.Contenido.sel_marcas.txtCodigo.Enabled = .F.
	Thisform.Contenido.sel_marcas.valcpoid = 0
	Thisform.Contenido.sel_marcas.blanquear()
							...
							IF UPPER(ALLTRIM(columnas.fieldName)) == "CODARTPV" .OR. UPPER(ALLTRIM(columnas.fieldName)) == "CODART" THEN
								...
								ELSE
									...
									DO WHILE !EOF("cur_tmp")
										Thisform.llenar_cursor_registro()
										SELECT cur_tmp
										SKIP
									ENDDO
								ENDIF
							...
							ELSE
								IF ALLTRIM(columnas.dataType) == "N" THEN
									...
									ELSE
										...
										IF llOk THEN
											* Agrego validación de tipos de datos para el precio
											if !(alltrim(type("m.precio")) == "N") then
												messagebox("El precio no tiene el formato numérico en la fila " + alltrim(str(lnFila)), 0+48, Thisform.Caption)
												loExcel.WorkBooks.Close()
												loExcel.Quit()
												RELEASE loExcel
												return .f.
											...
											* Si los artículos se encontraron actualizo los precios
											* en registros.
											UPDATE registros SET prListaPV = m.precio WHERE ALLTRIM(registros.codArtPV) == ALLTRIM(m.codArtPV)
										ENDIF
									ENDIF
								ENDIF
							ENDIF
...
DO WHILE !EOF("registros")
	IF registros.prListaPV <> 0 THEN
		...
		lcSql = "CALL arti_actupreGrabar ( " ;
			...
			+ "?pCodUsu, " ;
			+ "?pIdHostModi) "
		lcSql = loCommand.AddParameter(lcSql, "pIdArticulo", ALLTRIM(STR(registros.idArticulo)), .f., .f.)
		lcSql = loCommand.AddParameter(lcSql, "pPrLista", ALLTRIM(STR(registros.prListaPV, 10, 2)), .f., .f.)
		lcSql = loCommand.AddParameter(lcSql, "pBonif1", ALLTRIM(STR(registros.bonif1, 10, 2)), .f., .f.)
		lcSql = loCommand.AddParameter(lcSql, "pBonif2", ALLTRIM(STR(registros.bonif2, 10, 2)), .f., .f.)
		lcSql = loCommand.AddParameter(lcSql, "pBonif3", ALLTRIM(STR(registros.bonif3, 10, 2)), .f., .f.)
		lcSql = loCommand.AddParameter(lcSql, "pBonif4", ALLTRIM(STR(registros.bonif4, 10, 2)), .f., .f.)
		lcSql = loCommand.AddParameter(lcSql, "pCostoRep", ALLTRIM(STR(lnCostoRep, 10, 2)), .f., .f.)
		lcSql = loCommand.AddParameter(lcSql, "pMargenMax", ALLTRIM(STR(registros.margenMay, 10, 2)), .f., .f.)
		lcSql = loCommand.AddParameter(lcSql, "pMargenMin", ALLTRIM(STR(registros.margenMin, 10, 2)), .f., .f.)
		lcSql = loCommand.AddParameter(lcSql, "pPrVentaMax", ALLTRIM(STR(lnPrVentaMay, 10, 2)), .f., .f.)
		lcSql = loCommand.AddParameter(lcSql, "pPrVentaMin", ALLTRIM(STR(lnPrVentaMin, 10, 2)), .f., .f.)
		lcSql = loCommand.AddParameter(lcSql, "pPrFinalMax", ALLTRIM(STR(lnPrVtaFinalMay, 10, 2)), .f., .f.)
		lcSql = loCommand.AddParameter(lcSql, "pPrFinalMin", ALLTRIM(STR(lnPrVtaFinalMin, 10, 2)), .f., .f.)
		lcSql = loCommand.AddParameter(lcSql, "pCodUsu", ALLTRIM(gcCodUsu), .t., .f.)
		lcSql = loCommand.AddParameter(lcSql, "pIdHostModi", ALLTRIM(SYS(0)), .t., .f.)
...
loTherm.Complete()
MESSAGEBOX("Se detectaron " + ALLTRIM(STR(lnContarCeros)) + " artículos con precios cero.", 0+64, Thisform.Caption)
...
RETURN -1
ENDPROC
PROCEDURE llenar_cursor_registro
***************************************************************************************
* Permite llenar el cursor de registros a procesar.
* Fecha; 06/03/2023
* Desarrollador: Leonardo D. Zulli
***************************************************************************************

LOCAL m.idArticulo, m.codArt, m.descripcion, m.alicIVA
LOCAL m.bonif1, m.bonif2, m.bonif3, m.bonif4
LOCAL m.margenMin, m.margenMay, m.prListaAnt
LOCAL m.precio, m.codArtPV

m.precio = 0.00 && Lo dejo en cero porque este dato lo voy a tener cuando llegue a la columna precio

SELECT cur_tmp
m.idArticulo = cur_tmp.idArticulo
m.codArt = ALLTRIM(cur_tmp.codArt)
m.codArtPV = ALLTRIM(cur_tmp.codArtPV)
m.descripcio = ALLTRIM(cur_tmp.descripcio)
m.alicIVA = cur_tmp.alicIVA
m.bonif1 = IIF(Thisform.contenido.txtBonif1.Value = 0, cur_tmp.bonif1, Thisform.contenido.txtBonif1.Value)
m.bonif2 = IIF(Thisform.contenido.txtBonif2.Value = 0, cur_tmp.bonif2, Thisform.contenido.txtBonif2.Value)
m.bonif3 = IIF(Thisform.contenido.txtBonif3.value = 0, cur_tmp.bonif3, Thisform.contenido.txtBonif3.Value)
...
&& Para agregar la fila me aseguro de que haya un código de artículo
