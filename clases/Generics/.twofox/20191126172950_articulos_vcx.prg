...
lcSql = lcSql + "SET prLista = " + ALLTRIM(STR(Thisform.txtNewPrLista.Value, 10, 2)) + ", "
lcSql = lcSql + "	bonif1 = " + ALLTRIM(STR(Thisform.txtNewBonif1.Value, 10, 2)) + ", "
lcSql = lcSql + "	bonif2 = " + ALLTRIM(STR(Thisform.txtNewBonif2.Value, 10, 2)) + ", "
lcSql = lcSql + "	bonif3 = " + ALLTRIM(STR(Thisform.txtNewBonif3.Value, 10, 2)) + ", "
lcSql = lcSql + "	bonif4 = " + ALLTRIM(STR(Thisform.txtNewBonif4.Value, 10, 2)) + ", "
lcSql = lcSql + "	costoRep = " + ALLTRIM(STR(Thisform.txtNewCosto.Value, 10, 2)) + ", "
lcSql = lcSql + "	margenMax = " + ALLTRIM(STR(Thisform.txtNewPorMay.Value, 10, 2)) + ", "
lcSql = lcSql + "	margenMin = " + ALLTRIM(STR(Thisform.txtNewPorMin.Value, 10, 2)) + ", "
lcSql = lcSql + "	prVentaMax = " + ALLTRIM(STR(Thisform.txtNewPrVtaMay.Value, 10, 2)) + ", "
lcSql = lcSql + "	prVentaMin = " + ALLTRIM(STR(Thisform.txtNewPrVtaMin.Value, 10, 2)) + ", "
lcSql = lcSql + "	prFinalMax = " + ALLTRIM(STR(Thisform.txtNewPrFinalMay.Value, 10, 2)) + ", "
lcSql = lcSql + "	prFinalMin = " + ALLTRIM(STR(Thisform.txtNewPrFinalMin.Value, 10, 2)) + " "
lcSql = lcSql + "WHERE idArticulo = " + ALLTRIM(STR(Thisform.idarticulo))
...
&& Genero el registro correspondiente al cambio de precio en la tabla art_cbios
&& para registrar los cambios.

IF Thisform.txtPrLista.Value <> 0 THEN
	* lnVar = (1 - (Thisform.txtPrLista.Value / Thisform.txtNewPrLista.Value)) * 100
	lnVar = ((Thisform.txtNewPrLista.Value - Thisform.txtPrLista.Value) / Thisform.txtPrLista.Value) * 100
ELSE
	lnVar = 0.00
ENDIF
...
IF lnVar <> 0 THEN
	...
	lcSql = lcSql + ALLTRIM(STR(Thisform.txtPrLista.Value, 10, 2)) + ", "
	lcSql = lcSql + ALLTRIM(STR(Thisform.txtNewPrLista.Value, 10, 2)) + ", "
	lcSql = lcSql + ALLTRIM(STR(lnVar, 10, 2)) + ")"
...
&& Si se está editando el precio desde ingresos, entonces, marco como
&& controlado el registro correspondiente a stk_ingdet
IF Thisform.actualiza_ingresos THEN
	lcSql = "UPDATE stk_ingdet "
	lcSql = lcSql + "SET procesado = 1 "
	lcSql = lcSql + "WHERE idIngDet = " + ALLTRIM(STR(Thisform.idingdet)) + " "
	lcSql = lcSql + "	AND idIngCab = " + ALLTRIM(STR(Thisform.idingcab))
...
MESSAGEBOX("El artículo ha sido modificado con éxito", 0+64, Thisform.Caption)
...
LOCAL m.clienteDD, m.clienteHH, m.fechaDD, m.fechaHH
LOCAL loDT
...
IF lnIdClienteDD <> 0 .AND. lnIdClienteHH <> 0 THEN
	lcSql = lcSql + " AND clientes.idCliente BETWEEN " + ALLTRIM(STR(lnIdClienteDD)) + " AND " + ALLTRIM(STR(lnIdClienteHH)) + " AND "
...
ELSE
	lcSql = lcSql + "faltantes.fecAlta >= " + loDT.toMySql(Thisform.Contenido.txtFechaDD.Value ) + " AND "
	lcSql = lcSql + "faltantes.fecAlta <= " + loDT.toMySql(Thisform.Contenido.txtFechaHH.Value) + " "
ENDIF
...
IF RECCOUNT("cur_x") > 0 THEN
	GO TOP
ELSE
	MESSAGEBOX("No hay registro para mostrar", 0+64, Thisform.Caption)
	loRes.Close_Query()
	RETURN .F.
ENDIF
...
lcSql = "SELECT * FROM codiart WHERE idArticulo = " + ALLTRIM(STR(cur_artic.idArticulo)) + " "
lcSql = lcSql + "AND (circuito = 'C' OR circuito = 'CV')"
loRes.ActiveConnection = goConn.ActiveConnection
loRes.Cursor_Name = "cur_unidesp"
...
DO WHILE !EOF("cur_unidesp")
	THISFORM.cboUnidCpa.AddItem(ALLTRIM(STR(cur_unidesp.cantiDesp)))
...
*!*	lcbuscar = STRTRAN(ALLTRIM(Thisform.contenido.txtvalorbuscado.Value)," ","*")

*!*	SELECT cur_Artic
*!*	SET FILTER TO (LIKE('*' + ALLTRIM(lcbuscar) + '*', cur_Artic.CodArt);
*!*		OR LIKE('*' + ALLTRIM(lcbuscar) + '*', cur_Artic.Descripcio) ; 
*!*		OR LIKE('*' + ALLTRIM(lcbuscar) + '*', cur_Artic.descMarca) ;
*!*		OR LIKE('*' + ALLTRIM(lcbuscar) + '*', cur_Artic.descsubfam)) ;
*!*	IN cur_Artic	
