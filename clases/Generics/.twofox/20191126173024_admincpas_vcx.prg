...
Thisform.contenido.txtTotalOP.Value = lnImpEfvo + lnImpCheques + lnTotChqP + lnImpDepo + lnRetIVA + lnRetIIBB + lnRetGan + lnRetSUSS
Thisform.contenido.txtResto.Value = lnImpOP - lnImpEfvo - lnImpCheques  - lnTotChqP - lnImpDepo - lnRetIVA - lnRetIIBB - lnRetGan - lnRetSUSS
...
DO WHILE !EOF("cheq_disp")
	IF cheq_disp.sel THEN 
		...
		m.detcheq2 = m.detcheq2  + IIF(ALLTRIM(cheq_disp.chq_nro) == "", SPACE(8), ALLTRIM(cheq_disp.chq_nro)) + SPACE(25)
		
		m.detcheq2 = m.detcheq2 + IIF(cheq_disp.fecEmis = {} .OR. ISNULL(cheq_disp.fecEmis), SPACE(10), ALLTRIM(DTOC(cheq_disp.fecEmis))) + SPACE(20)
...
lcSql = lcSql + "WHERE planctas.codAbr IN ('CJP', 'CJD', 'CJC') "
...
*!*		IF ALLTRIM(ordpag_d.tipoPago) == "CHEQP"
*!*		
...
*!*			lcSql = lcSql + ALLTRIM(STR(chequesp.importe, 10, 2)) + ", "
*!*			lcSql = lcSql + "'" + ALLTRIM(gcCodUsu) + "', "
...
*!*			lcSql = lcSql + "SET chequeras.numActual = " + ALLTRIM(STR(chequesp.nroCheque)) + " + 1 "
...
&& Paso 3 - Tengo que grabar los registros de vinculación de OP con FC.
SELECT comprob
GO TOP
DO WHILE !EOF("comprob")
	IF comprob.sel THEN
...
&& Paso 4 - Tengo que grabar los registros de cuentas corrientes.
This.aplicar_cbtes()
...
&& Paso 5 - Grabar los cheques propios en la tabla de cheques propios
SELECT chequesp
GO TOP
DO WHILE !EOF("chequesp")
	lnIdChequeP = goConn.GetNextID("chequesp", "idChequeP")
...
lcSql = loRes.AddParameter(lcSql, "_idTransp", "38", .f., .f.)
...
lcFileName = "citicpa-cabecera-" + ALLTRIM(STR(YEAR(thisform.txtFechaDD.Value))) ;
	+ ALLTRIM(STR(MONTH(thisform.txtFechaHH.Value))) + ".txt"
...
lcSql = "CALL citicpas_obtenerAlicuotas ()"
loRes = CREATEOBJECT("odbc_result")
loRes.ActiveConnection = goConn.ActiveConnection
loRes.Cursor_Name = "cur_citialic"
loRes.OpenQuery(lcSql)
lcFileName = "citicpa-alicuotas-" + ALLTRIM(STR(YEAR(thisform.txtFechaDD.Value))) ;
	+ ALLTRIM(STR(MONTH(thisform.txtFechaHH.Value))) + ".txt"
lcFileName = PUTFILE("Guardar como:", lcFileName, "txt")
lnFileHandler = FCREATE(lcFileName)
