...
lnIdFaltante = goConn.getNextID("faltantes", "idFaltante")

lcSql = "insert into faltantes ( "
lcSql = lcSql + "	idFaltante, "
...
lcSql = lcSql + "values ( "
lcSql = lcSql + ALLTRIM(STR(lnIdFaltante)) + ", "
lcSql = lcSql + ALLTRIM(STR(tnIdArticulo)) + ", "
lcSql = lcSql + "'" + ALLTRIM(tcCodArt) + "', "
lcSql = lcSql + ALLTRIM(STR(tnUniCpr, 10, 2)) + ", "
lcSql = lcSql + ALLTRIM(STR(tnCantidad, 10, 2)) + ", "
lcSql = lcSql + "0,"

IF tnIdCliente <> 0 THEN
	lcSql = lcSql + ALLTRIM(STR(tnIdCliente)) + ", "
ENDIF
...
lnProxID = loNumerador.numActual
...
DO WHILE !EOF("cur_tempo") 
	...
	REPLACE idOcCab WITH cur_tempo.idOcCab 
 	...
 	REPLACE idMarca WITH IIF(lnSqlSrv = 1,cur_tempo.idMarca,INT(VAL(cur_tempo.idMarca))) ADDITIVE
