...
Thisform.Dialogresult = 2
*DO FORM (Thisform.nombre_form_nuevo)
loForm = CREATEOBJECT(Thisform.nombre_form_nuevo)
loForm.show()
IF loForm.registro_nuevo_ok THEN
	Thisform.txtValorBuscado.Value = ALLTRIM(loForm.txtDescripcion.value)
	Thisform.buscar_registros()
ENDIF
Thisform.Hide()
ENDPROC
...
&& Valido que la cantidad de caracteres de búsqueda no supere los 255.
if len(this.texto_busqueda) >= 255 then
	messagebox("La cantidad de caracteres de búsqueda no puede superar los 255", 0+48, "Búsqueda en grilla")
	return .f.
endif
...
ELSE
	...
	ELSE
		&& Si el usuario agregó un registro nuevo
		SELECT (SUBSTR(this.nombre_tabla, 1, 10))
		SET FILTER TO
		GO BOTTOM
		c_referencia = SUBSTR(this.nombre_tabla, 1, 10) + "." + This.nombre_campo_codigo
