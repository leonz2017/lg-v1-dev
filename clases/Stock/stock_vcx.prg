...
Thisform.contenido.txtFecDesde.Value = DATE(2014,01,01)
Thisform.contenido.txtFecHasta.Value = DATE()
Thisform.contenido.txtFecDesde.Enabled = .F.
Thisform.contenido.txtFecHasta.Enabled = .F.
Thisform.cargar_combos()
Thisform.contenido.txtFecDesde.SetFocus()
...
Thisform.contenido.txtFecDesde.Enabled = .T.
thisform.contenido.txtFecHasta.Enabled = .T.
ENDPROC
...
IF Thisform.contenido.opgStock.option1.Value = 1 THEN
	...
	&& Ajuste para ver el total de existencia de todos los articulos.
		SELECT SUM(existencia) as total_existencia FROM cur_MovStock INTO CURSOR totExistencia
		lnTotalExistencia = totExistencia.total_existencia
