***************************************************************
* Configuro los parámetros de global_cfg de FTP para probar
***************************************************************
...
goConn.ConnectionString = "DRIVER={MySQL ODBC 5.1 Driver};SERVER=localhost;PORT=3306;UID=root;PWD=l4ld81;DATABASE=siscom_mgc1;OPTION=3;"
...
addOrUpdateGlobalCFG("FTPHOST", "C", "168.181.185.95", .T.)
addOrUpdateGlobalCFG("FTPUSER", "C", "root", .T.)
addOrUpdateGlobalCFG("FTPPASS", "C", "03MGCar0719@", .T.)
addOrUpdateGlobalCFG("FTPPORT", "C", "5885", .T.)
addOrUpdateGlobalCFG("FTPINIDIR", "C", "/var/www/html/mg_server_updater/data/Temp", .T.)
addOrUpdateGlobalCFG("USA_SFTP", "L", "true", .T.)

goConn.Close()

goConn = CREATEOBJECT("odbc_connect")
goConn.ConnectionString = "DRIVER={MySQL ODBC 5.1 Driver};SERVER=localhost;PORT=3306;UID=root;PWD=l4ld81;DATABASE=siscom_mgc2;OPTION=3;"
WAIT WINDOW goConn.ActiveConnection NOWAIT
IF !goConn.Open()
	MESSAGEBOX(goConn.ErrorMessage, 1+16, "Database Connection")
	RETURN .F.
...
goConn = CREATEOBJECT("odbc_connect")
goConn.ConnectionString = "DRIVER={MySQL ODBC 5.1 Driver};SERVER=localhost;PORT=3306;UID=root;PWD=l4ld81;DATABASE=siscom_mgs1c1;OPTION=3;"
WAIT WINDOW goConn.ActiveConnection NOWAIT
IF !goConn.Open()
	MESSAGEBOX(goConn.ErrorMessage, 1+16, "Database Connection")
	RETURN .F.
...
goConn = CREATEOBJECT("odbc_connect")
goConn.ConnectionString = "DRIVER={MySQL ODBC 5.1 Driver};SERVER=localhost;PORT=3306;UID=root;PWD=l4ld81;DATABASE=siscom_mgs1c2;OPTION=3;"
WAIT WINDOW goConn.ActiveConnection NOWAIT
IF !goConn.Open()
	MESSAGEBOX(goConn.ErrorMessage, 1+16, "Database Connection")
	RETURN .F.
...
goConn.close()
WAIT WINDOW "FTP para testing listo" NOWAIT