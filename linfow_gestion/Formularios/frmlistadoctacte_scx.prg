...
ENDPROC
PROCEDURE resumen
PARAMETERS lnIdCliDesde, lnIdCliHasta, lnIdVendDesde, lnIdVendHasta
...
DO WHILE !EOF("cur_sdm")
	...
	REPLACE cur_salmora.codPostal WITH IIF(ISNULL(cur_sdm.codPostal), "", cur_sdm.codPostal) ADDITIVE
	REPLACE cur_salmora.localidad WITH IIF(ISNULL(cur_sdm.localidad), "", cur_sdm.localidad) ADDITIVE
	REPLACE cur_salmora.telefono WITH IIF(ISNULL(cur_sdm.telefono), "", cur_sdm.telefono) ADDITIVE
	
    lcMail = IIF(ISNULL(cur_sdm.mail) OR EMPTY(cur_sdm.mail), "", cur_sdm.mail)
    IF EMPTY(lcMail)
        lcMail = ""
    ENDIF
    REPLACE cur_salmora.mail WITH lcMail ADDITIVE
	
	&& REPLACE cur_salmora.mail WITH IIF(ISNULL(cur_sdm.mail), "", cur_sdm.mail) ADDITIVE
...
IF Thisform.contenedor.contenedor2.optiongroup2.optResumen.Value = 1
	IF Thisform.resumen(lnIdCliDesde, lnIdCliHasta, lnIdVendDesde, lnIdVendHasta)
		SELECT cur_resumen
...
WITH _screen.oFoxyPreviewer	
	...
	.cEmailTo = ALLTRIM(LOWER(Thisform.mailfc))
