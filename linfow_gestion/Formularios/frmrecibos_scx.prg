...
m.leyenda = ""
lcRepName = thisform.recibos.repname
...
DO WHILE !EOF("cur_facturas")
	IF cur_facturas.selected THEN
		...
		* 31/10/2023: Amplio el campo a 20, 2. Antes era 9 - LEN()
		m.detfact = m.detfact + REPLICATE(" ", 20 - LEN(ALLTRIM(STR(cur_facturas.impApl, 20, 2)))) + ALLTRIM(STR(cur_facturas.impApl, 20, 2))
...
DO WHILE !EOF("cur_detalle")
	...
	** Valido si es una transferencia bancaria.
	IF ALLTRIM(cur_detalle.tipoPago) == "TRB" THEN
		** Si es transferencia bancaria imprimo el número de 
		m.detpag = m.detpag + IIF(ALLTRIM(cur_detalle.nroTransf) == "", SPACE(8), ALLTRIM(cur_detalle.nroTransf)) + SPACE(1)
	ELSE
		** Si no es transferencia bancaria imprimo el número de cheque u 8 espacios
	...
	* 31/10/2023: Amplio el campo a 20, 2. Antes era 9 - LEN()
	m.detpag = m.detpag + REPLICATE(" ", 20 - LEN(ALLTRIM(STR(cur_detalle.importe, 20, 2)))) + ALLTRIM(STR(cur_detalle.importe, 20, 2)) + lcSaltoLinea
...
** Pregunto si quiere imprimir el recibo en caso de que esté configurado el
** parámetro RECIMPSEL.
IF getGlobalCFG("RCIMPSEL") THEN
	lnResp = MESSAGEBOX("¿Desea imprimir el recibo?", 32+4, Thisform.Caption)
	IF lnResp = 6 THEN
		* Paso por acá si usa el mensaje de confirmación de impresión.
	...
	ENDIF
ELSE
	* Si pasa por acá respeta el circuito del checkbox imprimir.
...
IF Thisform.pgf.page1.chkEnviarMail.Value THEN
	lcFileName = getGlobalCFG("FACPDFPATH") + "\RC_" + m.nroRec + ".pdf"
...
&& Si está configurador el paramétro de confirmación de impresión entonces,
&& oculto el checkbox imprimir para evitar redundancia de opciones.
&& Esto es por el cliente ERA puntualmente.
IF getGlobalCFG("RCIMPSEL") THEN
	Thisform.pgf.page1.chkImprimir.Visible = .F.
ENDIF
ENDPROC
...
** Valido que si se está ingresando una transferencia bancaria que cargue
** el número de transferencia.
IF this.Parent.opgTipoPago.option3.Value = 1 THEN
	IF ALLTRIM(this.Parent.txtnroTransferencia.Value) == "" THEN
		MESSAGEBOX("Debe ingresar el número de transferencia bancaria", 0+48, Thisform.Caption)
		This.Parent.txtNRoTransferencia.SetFocus()
		RETURN .F.
	ENDIF
ENDIF
...
IF !llEsCheque THEN
	...
	IF this.Parent.opgTipoPago.option3.Value = 1 THEN
		REPLACE cur_Detalle.tipoPago WITH "TRB" ADDITIVE
		REPLACE cur_Detalle.nroTransf WITH This.Parent.txtNroTransferencia.Value ADDITIVE
	ELSE
...
This.Parent.txtNroTransferencia.Value = ""
...
Option3.FontSize = 8
Option3.BackStyle = 0
Option3.Caption = "Transferencia Bancaria"
Option3.Height = 17
Option3.Left = 222
Option3.Top = 4
Option3.Width = 146
...
IF This.Value = 1 THEN
	...
	Thisform.Pgf.Page2.contenedor_cheques.txtNroTransferencia.Enabled = .F.
ENDIF
ENDPROC
...
IF This.Value = 1 THEN
	...
	Thisform.Pgf.Page2.contenedor_cheques.txtNroCUIT.Enabled = .F.
	Thisform.Pgf.Page2.contenedor_cheques.txtTitular.Enabled = .F.
	Thisform.Pgf.Page2.contenedor_cheques.txtNroTransferencia.Enabled = .T.
ENDIF
ENDPROC
