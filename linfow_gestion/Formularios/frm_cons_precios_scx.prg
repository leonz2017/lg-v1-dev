...
IF ALLTRIM(GetConfig("USA_FISCAL")) == "S" THEN
	Thisform.cbte_m2 = CREATEOBJECT("cls_cbtes_m2")			&& Genera el objeto que contiene el formulario del comprobante con soporte a impresora fiscal.
ELSE
	Thisform.cbte_m2 = CREATEOBJECT("cls_cbtes_m2_sf")		&& Genera el objeto que contiene el formulario del comprobante para uso sin impresora fiscal
ENDIF

Thisform.cbte_m2.cbte = ALLTRIM(getConfig("CARTCBTE"))	&& Indica que tipo de comprobante se va a emitir desde la consulta de artículos.
...
Thisform.cbte_m2.Caption = "Ingreso de Facturas"		&& Establece el título de la ventana.
Thisform.WindowState = 2

&& Verifico si el sistema tiene la funcionalidad de facturación
IF !getGlobalCFG("FAC_MODULE") THEN
	Thisform.Contenido.cnt_command_facts.Visible = .F.
ELSE
	Thisform.Contenido.cnt_command_facts.Visible = .T.
ENDIF
...
Alignment = 2
BackStyle = 1
Caption = "Acciones de facturación"
Height = 15
Left = 1
Top = 1
Width = 297
...
Thisform.cbte_m2.contenido.btnGrabar.Enabled = .T.
...
Height = 24
Width = 214
Caption = "\<Mostrar todos los precios posibles"
