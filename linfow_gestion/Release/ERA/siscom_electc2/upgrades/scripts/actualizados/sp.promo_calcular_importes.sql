    ...
    */
    ...
    INSERT INTO promo_cli (
        ...
        total_vend)
            ...
            WHERE
                cast(cab.fecEmision AS DATE) between cast(prc.vigencia_d AS DATE) AND cast(prc.vigencia_h AS DATE)
            GROUP BY
                prc.idpromo_c,
                cab.idCliente,
                cab.idVentasC,
    ...
    INSERT INTO promo_cli (
        ...
        total_vend)
            SELECT
                ...
                2 AS cuenta,
                prc.imp_tope,
                round(sum(totNeto), 2) as total_vend
            FROM
                linfowgst_desac2.ventasdet det
                    INNER JOIN linfowgst_desac2.ventascab cab 
                        ON cab.idVentasC = det.idVentasC
                    INNER JOIN articulos art 
                        ON art.idArticulo = det.idArticulo
                    INNER JOIN promo_marcas prm
